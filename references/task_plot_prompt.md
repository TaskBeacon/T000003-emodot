...
- Neutral-Neutral: representative NN trials; both faces are neutral; female and male pools exist

Timeline phases:
- Positive-Neutral: Fixation (+; 0.8-1.0 s; no response) -> Face pair (positive face + neutral face; 0.5 s) -> ISI (+; 0.4-0.6 s) -> Dot probe (left or right dot; up to 1.0 s; press f for left, j for right)
- Sad-Neutral: Fixation (+; 0.8-1.0 s; no response) -> Face pair (sad face + neutral face; 0.5 s) -> ISI (+; 0.4-0.6 s) -> Dot probe (left or right dot; up to 1.0 s; press f for left, j for right)
- Neutral-Neutral: Fixation (+; 0.8-1.0 s; no response) -> Face pair (neutral face + neutral face; 0.5 s) -> ISI (+; 0.4-0.6 s) -> Dot probe (left or right dot; up to 1.0 s; press f for left, j for right)

Visual requirements:
- White background, landscape orientation, crisp dark text, restrained condition accent colors.
- One horizontal row per representative trial type.
- Each row contains exactly 4 participant-screen snapshots connected by a subtle arrow.
- Each screen snapshot shows visible participant content, not internal variable names.
- Use gray participant-screen boxes, thin black arrows, consistent row spacing, and subtle row separators.
- Use schematic face icons only: positive face, sad face, and neutral face. Do not use realistic people or photos.
- In the dot-probe screen, show both possible probe sides as a compact split note: left dot -> f; right dot -> j.
